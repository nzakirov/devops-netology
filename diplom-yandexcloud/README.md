...
В базовом тарифе YandexCloud внешние ip адреса выадются облачным провайдером динамически, при остановке и старте ВМ каждый раз получаем новый  внешний ip. Для того чтобы в доменной зоне обновлялись А записи при каждом старте был  написан скрипт [set-dns-records.sh](./src/set-dns-records.sh), который получает новый внешний ip и меняет записи внутри зоны.
